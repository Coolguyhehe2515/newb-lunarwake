...
float cloudBoxNoise2D(
  ...
  for (int i=0;i<octaves;i++) {
    a = mix(
      a,
      1.0,
      step(threshold,floor(rand(floor(p))))
    );
...
vec4 renderCloudsBox(
  nl_skycolor skycol,
  vec3 pos,
  highp float t,
  float rain
) {
  float d = cloudBoxNoise2D(
    pos.xz,
    t,
    rain,
    NL_CLOUDBOX_OCTAVES,
    NL_CLOUDBOX_THRESHOLD
  );
